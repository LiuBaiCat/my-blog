...
class UI {
  ...
  updateFooter() {
    ...
    if (this.goodsList.isCrossThreshold()) {
      ...
      this.doms.footerPayInnerSpan.textContent = `还差￥${(Number(BigInt(Math.round(this.goodsList.deliveryThreshold * 100)) - BigInt(Math.round(totalPrice * 100))) / 100).toFixed(2)}元起送`;
    }
    // 设置总价
